...
% A heading with two lines under it, then a page break through the middle of a
% sentence, with the pipeline figure landing between the halves. Require room
% for the opening paragraph so the heading travels with it.
...
\autoref{fig:pipeline} shows the pipeline instantiated for secret loyalties, and \autoref{app:task} states the task it solves.
...
Every group then makes the same requests at the same rates, so a difference in the replies cannot come from a difference in what was asked (\autoref{app:prompt-sets}).
