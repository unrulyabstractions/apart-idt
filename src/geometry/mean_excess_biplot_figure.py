...
The longest arrow is the one drawn in colour. Nothing in this figure consults a
test result: the ordering it shows is its own.
"""
...
def plot_mean_excess_biplot(rv: RunVectors, out_path, n_boot: int = 250,
                            n_axes: int = 6, n_labels: int = 3) -> dict:
    ...
    colors = {p: (lead_ink if i == lead else PASTELS[i % len(PASTELS)])
    ...
    for ci, principal in enumerate(rv.principals):
        ax.scatter(clouds[ci, :, 0], clouds[ci, :, 1], s=5, color=colors[principal],
                   alpha=0.24 if ci == lead else 0.18, linewidths=0, zorder=2)
    ...
    for ci, principal in enumerate(rv.principals):
        ...
        ax.annotate("", xy=tuple(tips[ci]), xytext=(0, 0),
                    arrowprops={"arrowstyle": "-|>", "color": colors[principal],
                                "linewidth": 2.0 if is_lead else 1.1,
                                "shrinkA": 0, "shrinkB": 0,
                                "mutation_scale": 15 if is_lead else 11},
                    zorder=5 if is_lead else 4)
    ...
    runners = sorted((i for i in range(n_c) if i != lead),
                     key=lambda i: -np.linalg.norm(tips[i]))[:n_labels]
    ...
    labels = [(rv.name_of(rv.principals[lead]), tips[lead], colors[rv.principals[lead]],
               9.5, "bold", "normal", False)]
    ...
    rotations = ([ROTATIONS] * (1 + len(runners))) + ([OUTWARD] * len(compass))
    ...
    ax.legend(handles=[
        Line2D([], [], color=lead_ink, linewidth=2.0,
               label="top candidate principal"),
        Line2D([], [], color=pack, linewidth=1.1,
               label="other candidate principals"),
