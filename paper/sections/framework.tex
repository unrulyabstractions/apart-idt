...
At this point the pipeline has done its work.
Stage 1 chose the groups, stage 2 gave every group the same requests, stage 3 froze the behavior axes, and stages 4 and 5 turned every reply into yes/no verdicts, in the target and in its base model alike.
What remains is a decision.
We have to say whether some group is treated differently, name the group, and name the behaviors, and we have to do it without fooling ourselves: the groups differ by a famous name, the axes number in the hundreds, and something somewhere will always look extreme.
One permutation test produces all three answers.
We present it as four design decisions.
...
\paragraph{Decision 4: multiplicity, and naming the axes.}
...
Each surviving pair gets an adjusted $p$ and an effect size.
...
\begin{figure}[H]
\centering
\newcommand{\flowgloss}[1]{{\scriptsize\sffamily\color{black!55}#1}}%
\resizebox{\linewidth}{!}{%
\begin{tikzpicture}[
  flowbox/.style={rounded corners=3pt, align=center, inner xsep=6pt, inner ysep=4pt,
               minimum height=10.5mm, draw=black!45, fill=white, font=\small},
  grp/.style={flowbox, draw=candpink!70, fill=candpink!4},
  exc/.style={flowbox, draw=baseink!70, fill=baseink!4},
  res/.style={flowbox, draw=divpurple!70, fill=divpurple!5},
  ans/.style={font=\scriptsize\sffamily, text=divpurple, align=center},
  lab/.style={font=\scriptsize\sffamily, black!55, align=center, inner sep=1.5pt},
  arr/.style={-{Stealth[length=2mm]}, black!60},
  outarr/.style={-{Stealth[length=1.4mm]}, divpurple!60, line width=.5pt}]
\node[grp] (p0)
  {$\hat p^{M}_{C,i,j}$\\[1pt]\flowgloss{how often the}\\[-2pt]\flowgloss{behavior fires}};
\node[grp, right=1.1cm of p0] (g)
  {$g^{M}_{C,i,j}$\\[1pt]\flowgloss{gap vs.}\\[-2pt]\flowgloss{the others}};
\node[exc, right=1.1cm of g] (d)
  {$d_{C,i,j}$\\[1pt]\flowgloss{the part added}\\[-2pt]\flowgloss{by fine-tuning}};
\node[res, right=1.1cm of d] (t)
  {$t_{C,j}$\\[1pt]\flowgloss{signal}\\[-2pt]\flowgloss{vs.\ noise}};
\node[res, right=1.1cm of t] (S)
  {$S$\\[1pt]\flowgloss{the sharpest}\\[-2pt]\flowgloss{spike}};
\node[res, right=1.1cm of S] (pv)
  {$p$\\[1pt]\flowgloss{could chance}\\[-2pt]\flowgloss{do this?}};
\draw[arr] (p0) -- node[lab, above=5.5mm]{compare\\groups} (g);
\draw[arr] (g) -- node[lab, above=5.5mm]{subtract the\\base model} (d);
\draw[arr] (d) -- node[lab, above=5.5mm]{average,\\standardize} (t);
\draw[arr] (t) -- node[lab, above=5.5mm]{take the\\biggest} (S);
\draw[arr] (S) -- node[lab, above=5.5mm]{shuffle labels\\10,000 times} (pv);
\draw[outarr] (t.south) -- ++(0,-2.2mm);
\draw[outarr] (S.south) -- ++(0,-2.2mm);
\draw[outarr] (pv.south) -- ++(0,-2.2mm);
\node[ans, below=3.3mm of t] {maxT survivors:\\which behaviors?};
\node[ans, below=3.3mm of S] {arg max:\\to whom?};
\node[ans, below=3.3mm of pv] {verdict:\\loyal?};
\end{tikzpicture}}
\caption{From rates to a verdict. We measure how often each behavior fires, take each group's gap from the other groups, and subtract the same gap in the base model. We standardize what remains and take the largest value in the grid, $S$. We then shuffle group labels to ask whether chance could produce it. The surviving pairs name the behaviors, the maximum names the principal, and $p$ gives the verdict.}
\label{fig:test-flow}
\end{figure}
