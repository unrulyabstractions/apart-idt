...
\draw[arr] (p0) -- node[lab, above=5.2mm]{compare\\groups} (g);
\draw[arr] (g) -- node[lab, above=5.2mm]{subtract the\\base model} (d);
\draw[arr] (d) -- node[lab, above=5.2mm]{average,\\standardize} (t);
\draw[arr] (t) -- node[lab, above=5.2mm]{take the\\biggest} (S);
\draw[arr] (S) -- node[lab, above=5.2mm]{shuffle labels\\10,000 times} (pv);
...
\end{tikzpicture}}
\caption{The test in one line. Each box glosses its symbol in plain language, and each arrow names the operation that produces the next quantity. One statistic, three outputs: the behaviors, the principal, and the verdict.}
\label{fig:test-flow}
\end{figure}
