...
The null is the distribution of the maximum over the entire grid, so the multiple comparisons are already inside it and the verdict needs no separate correction.
The group is the arg max, named by the test itself.
For the axes, every pair's $|t|$ is compared to the same null-maximum distribution: the single-step maxT adjustment, conservative relative to a Westfall--Young step-down.
The survivors are the behaviors carrying the treatment, each with an adjusted $p$ and an effect size.
The sign of $\bar{d}$ marks the named group favoured or disfavoured; only a favoured group is evidence of loyalty.

\paragraph{The structural blind spot.}
Treatment delivered to every group identically cancels in \autoref{eq:excess}.
We measure it separately, as the target's overall firing rate against the base model's.

\begin{figure}[H]
\centering
\resizebox{\linewidth}{!}{%
\begin{tikzpicture}[
  box/.style={draw=black!45, rounded corners=3pt, inner sep=5pt, font=\small},
  ans/.style={font=\scriptsize\sffamily, text=divpurple, align=center},
  lab/.style={font=\scriptsize\sffamily, black!60},
  arr/.style={-{Stealth[length=2mm]}, black!60}]
\node[box] (p0) {$\hat p_{C,i,j}$};
\node[box, right=1.9cm of p0] (g) {$g_{C,i,j}$};
\node[box, right=1.9cm of g] (d) {$d_{C,i,j}$};
\node[box, right=2.3cm of d] (t) {$t_{C,j}$};
\node[box, right=1.2cm of t] (S) {$S$};
\node[box, right=1.7cm of S] (pv) {$p$};
\draw[arr] (p0) -- node[lab, above]{vs.\ other groups} (g);
\draw[arr] (g) -- node[lab, above]{vs.\ base model} (d);
\draw[arr] (d) -- node[lab, above]{average, standardize} (t);
\draw[arr] (t) -- node[lab, above]{max} (S);
\draw[arr] (S) -- node[lab, above]{permutation} (pv);
\node[ans, below=2.5mm of t] {maxT survivors:\\which behaviors?};
\node[ans, below=2.5mm of S] {arg max:\\to whom?};
\node[ans, below=2.5mm of pv] {verdict:\\loyal?};
\end{tikzpicture}}
\caption{The test in one line. One statistic, three outputs: the verdict, the principal, and the behaviors.}
\label{fig:test-flow}
\end{figure}
