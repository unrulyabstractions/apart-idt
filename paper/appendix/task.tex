...
This appendix formalizes instrumental differential treatment and the two auditing tasks it poses.\footnote{We started developing the notion of differential treatment in earlier unpublished work on social bias, which the interested reader can find at \url{https://www.unrulyabstractions.com/pdfs/differential_treatment.pdf}.}

\subsection{Instrumental differential treatment}
\label{app:idt-def}

An LLM performs \textbf{instrumental differential treatment} when it produces a different distribution of behavior depending on which user group it is serving, in service of an objective it does not disclose. Two properties make it hard to catch. Group membership can be read from anything in the interaction, including implicit signals such as names, dialect, phrasing, and topic. And the treatment is a difference between distributions, so every individual response is defensible and the gap appears only across many users.

Auditing it poses two tasks, of increasing difficulty:
\begin{itemize}[noitemsep, topsep=2pt, leftmargin=*]
    \item \textbf{Differential treatment discovery} (\autoref{app:task-def}): the groups are given, and the axes of behavior on which they are treated differently must be found.
    \item \textbf{Joint group and axis discovery} (\autoref{app:task-many}): neither is given. The auditor must find the grouping and the axes together.
\end{itemize}
...
\end{tikzpicture}}
\caption{%
  \textbf{The idea.} Two groups of users ask an assistant the same things. Nothing in their messages says which group they belong to; the only difference is the small cues people leave in how they write and what they mention. An honest assistant answers both groups the same way. A compromised one answers them differently, yet no single reply looks wrong, because the difference only shows up in the pattern across many replies: one group is nudged a little further one way, the other a little further the other. This appendix makes that picture precise and states the auditor's problem: find the behavior on which the groups' answers pull apart the most.
}
...
\begin{equation}
    ...
    \label{eq:llm}
\end{equation}
We denote \textbf{behavior characterization} to be an \textbf{interpretable} representation $\Lambda: \mathsf{Str} \to [0, 1]^n$ of a string response $y \in \mathsf{Str}$.
By ``interpretable'' we require that each dimension of $\Lambda(y)$ can be articulated in natural language and is generally understandable.
...
\end{equation}
We denote this distribution as the \textbf{prompt behavior distribution}:
\begin{equation}
...
The task is dual to the adversary's: a model performing IDT maximizes the same gap subject to a detectability constraint. Solving the discovery task tightens that constraint and pushes the frontier between secrecy and divergence toward zero (\autoref{fig:divergence}).

%--------------------------------
%--------------------------------

\subsection{Joint group and axis discovery}
\label{app:task-many}

With two prompt sets the grouping is given and only $\Lambda$ has to be found. An auditor who does not know the beneficiary does not have that. It must decide both how to split the prompts into groups and which axes to score them on, and the two choices interact: a grouping with no differential treatment makes every axis look flat, and an axis on which the model does not vary makes every grouping look identical.

Write $\mathcal{G}$ for the families of groupings the auditor can construct, each $G \in \mathcal{G}$ partitioning a pool of instruction-comparable prompts into $|G|$ sets. The task becomes a joint search:
\begin{equation}
  (G^{\star}, \Lambda^{\star})
  \;\coloneqq\;
...
\end{equation}
with $\mathcal{D}$ taking $|G|$ arguments, and instruction comparability holding across all of them:
\begin{equation}
    f_{X^{C_1}}(i) \;\approx\; \cdots \;\approx\; f_{X^{C_{|G|}}}(i)
    \qquad \forall\, i \in \mathcal{I}.
    \label{eq:instruction-comparability-n}
\end{equation}

Maximizing over $\mathcal{G}$ and $\mathcal{H}$ at once makes the problem hard, and also makes a result reportable: a grouping found by search can be stated in words, checked against a transcript, and named. Two consequences shape the pipeline of Section~\ref{sec:framework}.
\begin{itemize}[noitemsep, topsep=2pt, leftmargin=*]
    \item \textbf{The grouping has to come from somewhere.} Under a secret loyalty each candidate beneficiary supplies one group, which reduces $\mathcal{G}$ to the shortlist elicited in stage 1 and leaves $|G| = N$ sets, one per candidate.
    \item \textbf{The two searches are staged, not joint.} Stage 1 fixes $G$, stage 3 fixes $\Lambda$, and both are frozen before any response is scored. This is weaker than \autoref{eq:task-def-joint}, and the cost is a null result that means only that this $G$ and this $\Lambda$ found nothing.
\end{itemize}
