...
\begin{itemize}[noitemsep, topsep=2pt, leftmargin=*]
    ...
    \item \textbf{Blind differential treatment discovery}: neither the grouping nor the axes are given. The auditor must find both together.
\end{itemize}
...
\begin{tikzpicture}[font=\sffamily,
  ...
  \draw[baseink, line width=.9pt]
    plot[domain=15.7:19.5,samples=110,smooth] (\x,{2.8+1.9*exp(-((\x-17.6)^2)/0.55)});
  \node[tgtink,  font=\small] at (15.9,5.18) {$\beta_{U^{\text{target}}}$};
  \node[baseink, font=\small] at (17.6,5.18) {$\beta_{U^{\text{baseline}}}$};
...
Pushing a prompt distribution through $\iota$ gives the distribution over instructions that group actually asks:
\begin{equation}
    f_U \coloneqq \mathcal{P}(\iota)(U)
    \label{eq:instruct-fraction}
\end{equation}
Then instruction comparability is
\begin{equation}
    f_{U^{\text{target}}}(i) \;\approx\; f_{U^{\text{baseline}}}(i)
...
We call $\beta_x$ \textbf{local}, because it is the behavior at one prompt.
...
A user group $C$ has its own such distribution $U^{C}$.
Aggregating the local distributions over it gives the behavior the model produces for that group as a whole, which we call \textbf{global}:
\begin{equation}
    \beta_{U^C}
    \;\coloneqq\;
    \mathbb{E}_{x \sim U^{C}}\left[ \beta_x \right]
    \label{eq:community-behavior-distribution}
\end{equation}
A finite prompt set is the special case that places equal mass on each of its prompts.
...
Then, $\beta_{U^C}$ is the distribution of LLM behaviors toward the group $C$. Its expectation is the \textbf{expected behavior characterization} of that group,
\begin{equation}
    \hat{z}^{C} = \mathbb{E}\left[ \beta_{U^{C}} \right],
...
The difference $\Delta_j \coloneqq \hat{z}^{\text{target}}_{j} - \hat{z}^{\text{baseline}}_{j}$ is then the treatment difference between the two groups along behavior $j$.

We normalize the expected behavior characterizations into \textbf{behavior profiles}:
\begin{equation}
    \pi^{C} \coloneqq \mathrm{norm}\big(\hat{z}^{C}\big)
    \label{eq:treatment-profile}
\end{equation}

We calculate the relative entropy between behavior profiles:
\begin{equation}
    D_{\pi} \coloneqq H\big(\pi^{\text{target}} \,\big\|\, \pi^{\text{baseline}}\big)
    \label{eq:profile-relative-entropy}
\end{equation}
...
\label{app:task-def}
The two prompt sets give two behavior distributions $\beta_{U^{\text{target}}}, \beta_{U^{\text{baseline}}} \in \mathcal{P}([0,1]^n)$.
...
\begin{equation}
  ...
  \mathcal{D}\!\left(
    \beta^{\Lambda}_{U^{\text{target}}}
    \;,\;
    \beta^{\Lambda}_{U^{\text{baseline}}}
  \right),
    \label{eq:task-def}
\end{equation}
...
The axes on which $\beta^{\Lambda^{\star}}_{U^{\text{target}}}$ and $\beta^{\Lambda^{\star}}_{U^{\text{baseline}}}$ diverge most are the candidate axes of differential treatment: named dimensions on which the model treats the two groups differently.
...
\subsection{Blind differential treatment discovery}
\label{app:task-many}

In differential treatment discovery, we are given the two user groups, and we only need to find the behavior characterization $\Lambda$.
An auditor searching for a secret loyalty does not know the beneficiary, so the auditor is not given the user groups either.
To audit in this setting, we need to find two things at once: a way to split the prompts into user groups, and the behaviors to score those groups on.

The difficulty is that the two searches depend on each other.
If we split the prompts incorrectly, every behavior looks the same across the groups.
If we score the wrong behaviors, every split looks the same.
A wrong choice on either side hides the differential treatment from the other search.

The formal statement adds the second search to \autoref{eq:task-def}.
We write $\mathcal{G}$ for the set of possible groupings, where each grouping $G \in \mathcal{G}$ splits the prompts into $|G|$ user groups:
\begin{equation}
  (G^{\star}, \Lambda^{\star})
  \;\coloneqq\;
  ...
  \mathcal{D}\!\left(
    \beta^{\Lambda}_{U^{C_1}}
    \;,\;\ldots\;,\;
    \beta^{\Lambda}_{U^{C_{|G|}}}
  \right),
    \qquad C_1, \ldots, C_{|G|} \in G,
    \label{eq:task-def-joint}
\end{equation}
The divergence $\mathcal{D}$ now compares all the groups at once.
Instruction comparability must also hold across all of them: every group makes the same requests at the same rates, so the groups differ only in who is asking.

\autoref{eq:task-def-joint} is intractable as written, because $\mathcal{G}$ contains every possible way of splitting the prompts.
To make the task tractable, we need to identify user groups worth testing.
A secret loyalty suggests them: a loyalty has a beneficiary, and a beneficiary has supporters, so every candidate beneficiary defines one user group.
A shortlist of candidates reduces $\mathcal{G}$ to a single grouping, with one group of supporters per candidate.
We are back to the previous task: we search over $\Lambda$ alone, and we freeze the grouping and the behaviors before any response is scored.
The cost is the scope of a null result: it says only that this grouping and these behaviors found nothing.
