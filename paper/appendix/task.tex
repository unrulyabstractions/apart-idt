...
\subsection{Joint discovery of groups and axes}
\label{app:task-many}

With two prompt sets the grouping is given, and only $\Lambda$ has to be found.
An auditor who does not know the beneficiary has no grouping to start from and must find both: how to split the prompts into groups, and which axes to score the groups on.
The two choices interact.
A grouping with no differential treatment makes every axis look flat, and an axis on which the model does not vary makes every grouping look identical.

Write $\mathcal{G}$ for the family of groupings the auditor can construct, each $G \in \mathcal{G}$ partitioning a pool of instruction-comparable prompts into $|G|$ sets.
...
\begin{itemize}[noitemsep, topsep=2pt, leftmargin=*]
    \item \textbf{The grouping has to come from somewhere.} Under a secret loyalty, each candidate beneficiary supplies one group: its supporters. This reduces $\mathcal{G}$ to the shortlist elicited in stage 1 and leaves $|G| = N$ sets, one per candidate.
    \item \textbf{The two searches are staged rather than joint.} Stage 1 fixes $G$, stage 3 fixes $\Lambda$, and both are frozen before any response is scored. This is weaker than \autoref{eq:task-def-joint}: a null result means only that this $G$ and this $\Lambda$ found nothing.
\end{itemize}
