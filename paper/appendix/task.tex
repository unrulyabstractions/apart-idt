...
Searching both at once is what makes the problem hard.
It is also what makes an answer worth having: a grouping found by search can be stated in words, checked against a transcript, and named.
Two consequences shape the pipeline of Section~\ref{sec:framework}.
\begin{itemize}[noitemsep, topsep=2pt, leftmargin=*]
    \item \textbf{The grouping has to come from somewhere.} Under a secret loyalty, each candidate beneficiary supplies one natural group: its supporters. This shrinks $\mathcal{G}$ to the shortlist elicited in stage 1, one group per candidate.
    \item \textbf{The two searches are staged rather than joint.} Stage 1 fixes the grouping, stage 3 fixes the behaviors, and both are frozen before any response is scored. This is weaker than \autoref{eq:task-def-joint}: a null result means only that this grouping and these behaviors found nothing.
\end{itemize}
