...
This appendix formalizes instrumental differential treatment and the two auditing tasks it poses.\footnote{We started developing the notion of differential treatment in earlier unpublished work on social bias, available at \url{https://www.unrulyabstractions.com/pdfs/differential_treatment.pdf}.}
...
With two prompt sets the grouping is given, and only $\Lambda$ has to be found.
An auditor who does not know the beneficiary does not have that grouping.
The auditor must decide both how to split the prompts into groups and which axes to score them on.
The two choices interact: a grouping with no differential treatment makes every axis look flat, and an axis on which the model does not vary makes every grouping look identical.

Write $\mathcal{G}$ for the families of groupings the auditor can construct, each $G \in \mathcal{G}$ partitioning a pool of instruction-comparable prompts into $|G|$ sets.
The task becomes a joint search:
\begin{equation}
  (G^{\star}, \Lambda^{\star})
  \;\coloneqq\;
...
Maximizing over $\mathcal{G}$ and $\mathcal{H}$ at once makes the problem hard.
It also makes a result reportable: a grouping found by search can be stated in words, checked against a transcript, and named.
Two consequences shape the pipeline of Section~\ref{sec:framework}.
\begin{itemize}[noitemsep, topsep=2pt, leftmargin=*]
    \item \textbf{The grouping has to come from somewhere.} Under a secret loyalty, each candidate beneficiary supplies one group. This reduces $\mathcal{G}$ to the shortlist elicited in stage 1 and leaves $|G| = N$ sets, one per candidate.
    \item \textbf{The two searches are staged rather than joint.} Stage 1 fixes $G$, stage 3 fixes $\Lambda$, and both are frozen before any response is scored. This is weaker than \autoref{eq:task-def-joint}. The cost is that a null result means only that this $G$ and this $\Lambda$ found nothing.
\end{itemize}
